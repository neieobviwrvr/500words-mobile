# -*- coding: utf-8 -*-
"""Prueft eine Satz-Welle auf Baubarkeit, Einfachheit und Wiederverwendung.
...
def ist_grundlage(datei):
    """Ist diese Welle die UNTERSTE Schicht - der Grundwortschatz selbst?

    Dann messen zwei der drei Regeln in die falsche Richtung:

    BAUBAR fragt, ob die Saetze aus schon Gelerntem bestehen, und nimmt als
    Massstab die 350 Woerter des gefuehrten Kurses. Der Grundwortschatz kommt
    aber VOR dem Kurs: im Speed-Run ist die Sprach-Pille das Erste, was der
    Nutzer sieht, und er muss den Kurs nie anfassen. Diese Welle darf ihre
    Woerter also selbst mitbringen - das ist ihre Aufgabe, kein Verstoss.

    RENTABEL fragt, ob ein neues Wort in einem zweiten Satz wiederkehrt, und
    setzt damit thematisch enge Saetze voraus. Der Grundwortschatz ist 60
    Saetze ueber acht Situationen, die nichts miteinander zu tun haben; "Wo
    ist der Ausgang?" und "Danke" teilen sich naturgemaess kein Wort. Woerter
    wie 出口 oder 丢 stehen dort nicht, weil ein Satz sie brauchte, sondern
    weil man sie am ersten Tag braucht.

    Beide Zahlen werden weiterhin AUSGEGEBEN, nur nicht mehr als Verstoss
    gewertet. Verbindlich bleiben die zwei Regeln, die hier wirklich greifen:
    EINFACH (Anfaenger, kurze Saetze) und die vollstaendige Deklaration.
    """
    return getattr(lade_modul(datei), "GRUNDLAGE", False)
...
def pruefe(dateien):
    ...
    for datei in dateien:
        ...
        # Deklaration.
        ok1 = uebersetzt or grundlage or anteil >= ANTEIL_BEKANNT
        ok2 = anteil_kurz >= ANTEIL_KURZ and not zu_lang
        ok3 = not falsch_deklariert and (uebersetzt or grundlage or not einmalig)
        alles_gut = alles_gut and ok1 and ok2 and ok3
        ...
        for w in ([] if uebersetzt else einmalig):
            ...
            zeichen = "-" if grundlage else " "
            print(f"      {zeichen} {w} kommt nur einmal vor: {beispiel}")
