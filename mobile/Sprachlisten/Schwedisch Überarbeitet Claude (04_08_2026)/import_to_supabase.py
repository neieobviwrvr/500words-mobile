...
for entry in words:
    ...
    # Pruefen, ob das Wort in genau dieser Kategorie schon existiert
    # (Duplikat-Schutz ueber swedish+category statt nur swedish, weil es
    # echte Homonyme ueber Wortarten hinweg gibt, z.B. "man" als Pronomen
    # UND als Nomen "Ehemann" - beide sollen erhalten bleiben)
    existing = (
        supabase.table(TABLE_NAME)
        .select("id")
        .eq("swedish", swedish_text)
        .eq("category", entry.get("category"))
        .execute()
    )
    if existing.data:
        print(f"⏭  '{swedish_text}' ({entry.get('category')}) existiert schon, übersprungen.")
        skipped += 1
        continue
