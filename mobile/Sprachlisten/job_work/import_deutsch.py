# -*- coding: utf-8 -*-
"""Spielt die deutschen Job-Saetze nach phrasebook_master.

Anders als alles andere in dieser Sitzung schreibt dieses Skript in den
MASTER, nicht in eine Zielsprache - job_work wird geschrieben, nicht
uebersetzt. Die Uebersetzungen laufen danach ueber
uebersetzen/werkzeug.py wie bei jeder anderen Kategorie.

Prueft vor dem Schreiben:
  - kein Satz steht schon in derselben Kategorie (Idempotenz)
  - kein Satz kollidiert mit einer ANDEREN Kategorie, ausser er ist unten
    ausdruecklich erlaubt. Kategorieuebergreifende Dubletten gibt es zwar
    (zehn Stueck, absichtlich), aber jede weitere sollte eine Entscheidung
    sein und kein Versehen.
  - jedes Konzept hat mindestens ein Synonym, das im Satz vorkommt - sonst
    waere der Satz unloesbar (siehe uebersetzen/pruefe_konzepte.py)
...
# wuerden. Leer: der Entwurf wurde so gebaut, dass keine noetig ist.
...
def main():
    ...
    for s in M.SAETZE:
        ...
        if fremd and s["de"] not in ERLAUBTE_DUBLETTEN:
            ...
            continue
        # Loesbarkeit: jedes Konzept braucht ein Synonym, das im Satz steht
        t = s["de"].lower()
    ...
    import collections
    for sz, n in sorted(collections.Counter(s["sz"] for s in M.SAETZE).items()):
        print(f"  {sz:16} {n}")
    print()
    ...
    for i in range(0, len(zeilen), 40):
        rest(url, key, TABELLE, "POST", zeilen[i:i + 40])
